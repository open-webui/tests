...
def test_streamablehttp_client_is_context_manager_factory(depcheck):
    ...
    Note: 1.27.x marks `streamablehttp_client` deprecated in favour of
    `streamable_http_client`; the backend still imports the former (it
    remains functional), so we pin it and silence the bump-time warning."""
    mod = depcheck.load(IMPORT_NAME)
    fn = depcheck.resolve(mod, "client.streamable_http.streamablehttp_client")
